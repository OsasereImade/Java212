...
public class WordGUI extends JFrame {
	...
	public static void appendTextAreaText() {
        for (int i=0;i<Project3.originalText.length;i++) {
            myOriginalText.append(i+1 + "\t" + Project3.originalText[i] + "\n");
        }
	}
	
	//to construct a word line and store to an array list
	public static void appendTextAreaList() {
		ArrayList<WordLine> wordLineArray = new ArrayList<WordLine>();
		ListIterator listIter = Project3.b.listIterator(0); 
		int l = 1;
		while(listIter.hasNext()) {
			String word = listIter.next().toString();
			WordLine wl = new WordLine(word, l);
			wordLineArray.add(wl);
			l++;
		}
		...
		String str = "";
		for (int i = 0; i < Project3.b.size(); i++) {
			str += Project3.b.get(i) + "\n";
		}
